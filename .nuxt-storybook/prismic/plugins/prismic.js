...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://nicolegiaco.cdn.prismic.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"post","path":"/:uid"}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://nicolegiaco.cdn.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=nicolegiaco&new=true'
